using _1wireXamarinForms.CustomContainer;
using System;
...
namespace _1wireXamarinForms.DalSemi.OneWire.Adapter
{
    ...
    public abstract class PortAdapter : IDisposable
    {
		...
		/// <exception cref="OneWireException">On a setup error with the 1-Wire adapter</exception>  
		/// <seealso cref="_1wireXamarinForms.DalSemi.OneWire.Adapter.PortAdapter.IsPresent(byte[])"/>
		/// <seealso cref="Address"/>
		public void AssertSelectDevice(byte[] address)
		{
			if (!SelectDevice(address))
				throw new OneWireIOException("Device " + _1wireXamarinForms.DalSemi.OneWire.Utils.Address.ToString(address) + " not present.");
		}
		/// <summary>
		...
		{
			return GetDeviceContainer(_1wireXamarinForms.DalSemi.OneWire.Utils.Address.ToByteArray(address));
		}

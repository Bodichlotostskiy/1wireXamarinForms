...
namespace _1wireXamarinForms.DalSemi.OneWire.Container
{
    public class MemoryBankScratch : PagedMemoryBank, ScratchPad
    {
        ...
        /**
          ...
          */
        public const byte WRITE_SCRATCHPAD_COMMAND = 0x0F;

        /**
          * Read Scratchpad Command
          */
        public const byte READ_SCRATCHPAD_COMMAND = 0xAA;
        ...
        public static readonly byte[] ffBlock = { // Length: 96
         ...
         * @throws OneWireException
         */
        public virtual void ReadScratchpad(byte[] readBuf, int offset, int len,
                                    byte[] extraInfo)
        {
         ...
         * @throws OneWireException
         */
        public virtual void WriteScratchpad(int startAddr, byte[] writeBuf, int offset,
                                     int len)
        {
            ...
            raw_buf[2] = (byte)(((startAddr & 0xFFFF) >> 8) & 0xFF); // >> was >>>
        ...
        public void CheckSpeed()
        {
            lock (this)
            {
                ...
                // currently at the ibutton's max speed.  If it isn'thread, we should
                // call the doSpeed() method, since the adapter might have
                // changed speeds.
                if (doSetSpeed || (ib.Adapter.Speed != ib.GetMaxSpeed()))
                {
